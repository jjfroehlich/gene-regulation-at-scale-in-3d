#!/usr/bin/env python3
"""Build the canonical V4 gene-expression scene from arrangement V2."""
...
import canonical_v4_config as v4  # noqa: E402
import procedural_nucleic_geometry as nucleic_geometry  # noqa: E402


OUTPUT_DIR = v4.OUTPUT_DIR
BLEND_PATH = v4.BLEND_PATH
PREVIEW_PATH = v4.PREVIEW_PATH
REPORT_PATH = v4.REPORT_PATH
DETAIL_PREVIEWS = {
    "full_overview": OUTPUT_DIR / "preview_gene_expression_surface_style_v4_full_overview.png",
    "polymerase_rna_start": OUTPUT_DIR / "preview_gene_expression_surface_style_v4_polymerase_rna_start.png",
    "nucleosome_loop": OUTPUT_DIR / "preview_gene_expression_surface_style_v4_nucleosome_loop.png",
    "mrna_spiral": OUTPUT_DIR / "preview_gene_expression_surface_style_v4_mrna_spiral.png",
    "ribosome_top_translation": OUTPUT_DIR / "preview_gene_expression_surface_style_v4_ribosome_top_translation.png",
    "compact_mrna": OUTPUT_DIR / "preview_gene_expression_surface_style_v4_compact_mrna.png",
}
...
def build_v4_dna(manifest: dict, collections: dict, materials: dict) -> dict:
    path, report = direct_nucleic_meshes.build_dna_meshes(manifest, collections, materials)
    report["source_curve"] = add_source_path_curve("V4_DNA_source_path", path, collections["DNA"], materials["black"])
    base.create_text(
        "label_DNA_v4",
...
def build_v4_mrna(manifest: dict, collections: dict, materials: dict) -> dict:
    path, report = direct_nucleic_meshes.build_mrna_meshes(manifest, collections, materials)
    report["source_curve"] = add_source_path_curve("V4_mRNA_source_path", path, collections["mRNA"], materials["black"])
    start = path.points[0]
    base.create_text(
        "label_mRNA_v4",
...
def build_v4_compact_mrna(manifest: dict, collections: dict, materials: dict) -> dict:
    ...
    base.create_text(
        "label_compact_mrna_v4",
        "compact full-length mRNP reference",
...
def add_v4_cameras(manifest: dict, dna_path, mrna_path, compact_path, collections: dict, materials: dict) -> dict[str, str]:
...
def final_surface_contact_validation(
    ...
    strict_names = set(v4.DNA_STRICT_CONTACT_NAMES) | set(v4.RNA_STRICT_CONTACT_NAMES) | set(v4.RNA_BRACKET_CONTACT_NAMES)
...
def contact_validation(asset_reports: list[dict]) -> dict:
    ...
    strict_sets = {
        "dna": sorted(v4.DNA_STRICT_CONTACT_NAMES),
        "mrna": sorted(v4.RNA_STRICT_CONTACT_NAMES),
        "mrna_bracket": sorted(v4.RNA_BRACKET_CONTACT_NAMES),
    }
    ...
    display_rows = []
    for name in sorted(v4.DISPLAY_EXEMPT_NAMES):
        item = by_name.get(name)
        if not item:
            continue
        attachment = item.get("attachment_empty")
...
def validate_v4_report(report: dict) -> None:
    ...
    if failures:
        raise RuntimeError(f"Canonical V4 contact validation failed: {failures}")


def main() -> None:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    if not v4.MANIFEST_PATH.exists():
        v4.write_manifest()
    manifest = json.loads(v4.MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    dna = build_v4_dna(manifest, collections, materials)
    mrna = build_v4_mrna(manifest, collections, materials)
    compact_mrna = build_v4_compact_mrna(manifest, collections, materials)
    ...
    report = {
        ...
        "source_manifest": str(v4.MANIFEST_PATH),
    ...
    camera_names = add_v4_cameras(manifest, dna["path"], mrna["path"], compact_mrna["path"], collections, materials)
