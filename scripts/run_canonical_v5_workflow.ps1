...
Push-Location $Root
try {
    Invoke-Checked "Write canonical V5 manifest" {
        & $PythonExe "scripts\write_canonical_v5_manifest.py"
    }
    ...
    Invoke-Checked "Build canonical V5 Blender scene" {
        & $BlenderExe --background --python (Join-Path $Root "scripts\build_gene_expression_surface_scene_v5.py")
    }
}
finally {
    Pop-Location
