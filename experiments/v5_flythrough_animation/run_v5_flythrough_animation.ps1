...
$VideoName = $(if ($SmokeTest) { "v5_flythrough_animation_smoke.mp4" } elseif ($ReviewRender) { "v5_flythrough_animation_review.mp4" } else { "v5_flythrough_animation_1080p.mp4" })
...
$BlendPath = Join-Path $OutputDir "v5_flythrough_animation.blend"
...
try {
    ...
    if ($ReviewRender -and $encodedVideo) {
        Invoke-Checked "Build review contact sheet" {
            & $FfmpegExe -y -i $VideoPath -vf "fps=1/3,scale=320:-1:flags=lanczos,tile=5x4:padding=4:margin=4" -frames:v 1 -update 1 $ReviewContactSheet
        }
    }
