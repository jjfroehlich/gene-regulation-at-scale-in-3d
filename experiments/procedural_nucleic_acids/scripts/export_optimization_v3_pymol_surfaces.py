#!/usr/bin/env python3
"""Export DNA/RNA optimization V3 pseudoatom assets as PyMOL surface OBJs."""
...
ASSET_DIR = ROOT / "experiments" / "procedural_nucleic_acids" / "assets" / "optimization_v3"
CIF_DIR = ASSET_DIR / "cif"
RAW_DIR = ASSET_DIR / "raw_surfaces"
REPORT_PATH = RAW_DIR / "optimization_v3_pymol_surface_exports_report.json"
...
def main() -> None:
    ...
    calibrator = opt_geom.chirality_metric_from_1bna()
    assets = [
        opt_geom.build_dna_asset(manifest, calibrator),
        opt_geom.build_mrna_asset(manifest, compact=False),
        opt_geom.build_mrna_asset(manifest, compact=True),
    ]
    exports = []
    for asset in assets:
        print(f"Exporting DNA/RNA optimization V3 PyMOL surface {asset['asset_id']}")
        exports.extend(export_asset(asset))
    report = {
        "title": "DNA/RNA optimization V3 PyMOL surface exports",
        "coordinate_units": "angstrom",
        "angstrom_to_mm": opt_geom.ANGSTROM_TO_MM,
        "surface_quality": 0,
        "solvent_radius": 0.0,
        "exports": exports,
    }
