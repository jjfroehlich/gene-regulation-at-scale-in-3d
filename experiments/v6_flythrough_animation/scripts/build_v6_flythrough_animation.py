#!/usr/bin/env python3
"""Build and optionally render the V6 educational flythrough animation."""
...
DEFAULT_SOURCE_BLEND = ROOT / "outputs" / "canonical" / "gene_expression_surface_style_v6.blend"
DEFAULT_REPORT = ROOT / "outputs" / "canonical" / "gene_expression_surface_scene_v6_report.json"
DEFAULT_OUTPUT_DIR = ROOT / "experiments" / "v6_flythrough_animation" / "outputs"
...
def create_animation(args: argparse.Namespace, report: dict, output_blend: Path, output_mp4: Path, frames_dir: Path) -> dict:
    dna_points = curve_points("V6_DNA_source_path")
    mrna_points = curve_points("V6_mRNA_source_path")
...
def main() -> None:
    ...
    output_blend = as_path(args.output_blend) if args.output_blend else args.output_dir / "v6_flythrough_animation.blend"
    default_mp4 = {
        "smoke": "v6_flythrough_animation_smoke.mp4",
        "review": "v6_flythrough_animation_review.mp4",
        "final": "v6_flythrough_animation_1080p.mp4",
    }[args.render_profile]
    output_mp4 = as_path(args.output_mp4) if args.output_mp4 else args.output_dir / default_mp4
    report_path = args.output_dir / "v6_flythrough_animation_report.json"
    ...
    if not args.source_blend.exists():
        raise FileNotFoundError(f"Canonical V6 blend not found: {args.source_blend}")
    if not args.source_report.exists():
        raise FileNotFoundError(f"Canonical V6 report not found: {args.source_report}")
