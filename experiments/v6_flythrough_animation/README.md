# V6 Flythrough Animation

This experiment adds a continuous 66-second camera flight on top of the canonical V6 `.blend`.
...
```powershell
.\experiments\v6_flythrough_animation\run_v6_flythrough_animation.ps1
```

Run a fast low-resolution smoke test:

```powershell
.\experiments\v6_flythrough_animation\run_v6_flythrough_animation.ps1 -SmokeTest
```

Render the full 66-second review cut at 960×540 and 12 fps before committing to the final 1080p pass:

```powershell
.\experiments\v6_flythrough_animation\run_v6_flythrough_animation.ps1 -ReviewRender
```

Build the animation `.blend` and report without rendering video:

```powershell
.\experiments\v6_flythrough_animation\run_v6_flythrough_animation.ps1 -SkipVideoRender
```
...
Generated files are written under `experiments/v6_flythrough_animation/outputs/`, which is ignored by Git:

- `v6_flythrough_animation.blend`
- `v6_flythrough_animation_review.mp4`
- `v6_flythrough_animation_report.json`
- `review_contact_sheet.png`

The tracked README preview is:

- `docs/images/v6-flythrough-preview.gif`
...
- `0-5s`: moving full-scene overview at the canonical V6 lower oblique angle, with scale bars, DNA/mRNA brackets, and a direct pointer to actin.
